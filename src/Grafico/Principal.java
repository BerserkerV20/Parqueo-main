...
public class Principal extends javax.swing.JFrame implements Runnable {
    ...
    // 2. Implementa la clase clsExportarExcel
public class clsExportarExcel {
    public void exportarExcel(JTable tabla) {
        try {
            XSSFWorkbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("Registro Parqueo");
            ...
            // Guardar archivo
            try (FileOutputStream fileOut = new FileOutputStream("RegistroParqueo.xlsx")) {
...
public void generar(String nombre) throws FileNotFoundException, DocumentException {
    try {
        if (jTxtNombre.getText().isEmpty() || jTxtApellido1.getText().isEmpty() || jTxtPlaca.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Debe llenar como mínimo los campos de Nombre/Apellidos/Placa");
            return;
        }
        ...
        documento.add(new Paragraph("Placa del vehículo: " + jTxtPlaca.getText()));
    ...
    try {
        ...
        String posicion = modelo.getValueAt(filaSeleccionada, 7).toString();
        
        // Liberar el espacio en el mapa visual
        switch (posicion) {
            case "Posición 1":
        ...
        jTxtNombre.setText(modelo.getValueAt(filaSeleccionada, 0).toString());
        jTxtApellido1.setText(modelo.getValueAt(filaSeleccionada, 1).toString());
        jTxtApellido2.setText(modelo.getValueAt(filaSeleccionada, 2).toString());
        jTxtCedula.setText(modelo.getValueAt(filaSeleccionada, 3).toString());
        jTxtPlaca.setText(modelo.getValueAt(filaSeleccionada, 4).toString());

        // Configurar el tipo de vehículo
        String tipoVehiculo = modelo.getValueAt(filaSeleccionada, 6).toString();
        if (tipoVehiculo.equals("Carro")) {
            Vehiculo.setSelected(true);
        } else if (tipoVehiculo.equals("Moto")) {
            Moto.setSelected(true);
        }

        // Guardar datos originales
        datosOriginales = new String[]{
            modelo.getValueAt(filaSeleccionada, 0).toString(), // Nombre
            modelo.getValueAt(filaSeleccionada, 1).toString(), // Apellido1
            modelo.getValueAt(filaSeleccionada, 2).toString(), // Apellido2
            modelo.getValueAt(filaSeleccionada, 3).toString(), // Cédula
            modelo.getValueAt(filaSeleccionada, 4).toString(), // Placa
            modelo.getValueAt(filaSeleccionada, 6).toString(), // Tipo
            modelo.getValueAt(filaSeleccionada, 7).toString()  // Posición
        };

        // Confirmar edición
        int opcion = JOptionPane.showConfirmDialog(this,
            "¿Desea editar los datos de este vehículo?",
            "Confirmar Edición",
        ...
        if (opcion == JOptionPane.YES_OPTION) {
            ...
            JOptionPane.showMessageDialog(this,
                "Modifique los datos y presione 'Ingresar' para guardar los cambios");
            // Habilitar campos necesarios para edición
            jPIngresoVehiculo.setVisible(true);
        } else {
            // Si no se confirma la edición, limpiar los campos
            limpiarCampos();
        ...
        JOptionPane.showMessageDialog(this,
            "Error al editar la fila: " + e.getMessage(),
            "Error",
...
private void limpiarCampos() {
    ...
    String nombreUsuario = txtUsuarioverificar.getText().trim();
    ...
    if (!opcionAdmin.isSelected() && !opcionInvitado.isSelected()) {
        JOptionPane.showMessageDialog(rootPane, "Debe seleccionar el tipo de perfil");
        return;
    }

    // Determinar el tipo de perfil seleccionado
    String perfilSeleccionado = opcionAdmin.isSelected() ? "a" : "i";
    
    // Buscar el usuario en la lista
    boolean usuarioEncontrado = false;
    for (DatosdelRegistro usuario : Logica.usuariosLista) {
        // Verificar que coincidan usuario, contraseña y perfil
        if (usuario.getNombre().equals(nombreUsuario) && 
            usuario.getPass().equals(password) && 
            usuario.getPerfil().equals(perfilSeleccionado)) {
            
            usuarioEncontrado = true;
            
            // Mensaje de bienvenida
            String tipoUsuario = perfilSeleccionado.equals("a") ? "Administrador" : "Invitado";
            JOptionPane.showMessageDialog(this, 
                "Bienvenido al Sistema de " + tipoUsuario + "\n" + usuario.getNombre());
            ...
            // Si es invitado, ocultar opciones de administrador
            if (perfilSeleccionado.equals("i")) {
                jConfiguracion.setVisible(false);
    ...
    if (!usuarioEncontrado) {
        JOptionPane.showMessageDialog(rootPane, "Datos incorrectos o tipo de perfil incorrecto");
    }
}

// Método para debug - agregar en la clase Principal
private void mostrarUsuariosRegistrados() {
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
       try {
        ...
        exportador.exportarExcel(Tabla);
    } catch (Exception ex) {
        JOptionPane.showMessageDialog(this, 
