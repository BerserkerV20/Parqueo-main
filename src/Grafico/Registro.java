...
public class Registro extends javax.swing.JFrame {
 ...
 // Validar campos vacíos
    if (txtusuario.getText().isEmpty() || txtcontra.getText().isEmpty() || 
        RegistroNombre.getText().isEmpty() || RegistroCedula.getText().isEmpty()) {
        JOptionPane.showMessageDialog(rootPane, "Debe llenar todos los espacios");
        return;
    }

    // Validar selección de perfil
    if (!opcionAdminR.isSelected() && !opcionInvitadoR.isSelected()) {
        JOptionPane.showMessageDialog(rootPane, 
            "Debe seleccionar un tipo de perfil (Administrador/Invitado)");
        return;
    }

    String usuario = txtusuario.getText().trim();
    String password = txtcontra.getText().trim();
    String perfil = opcionAdminR.isSelected() ? "a" : "i";

    // Verificar si el usuario ya existe
    boolean usuarioExiste = false;
    for (DatosdelRegistro usr : Logica.usuariosLista) {
    ...
    DatosdelRegistro nuevoUsuario = new DatosdelRegistro();
    nuevoUsuario.setNombre(usuario);
    nuevoUsuario.setPass(password);
    nuevoUsuario.setPerfil(perfil);
    Logica.usuariosLista.add(nuevoUsuario);

    // Mensaje de éxito
    String tipoUsuario = perfil.equals("a") ? "Administrador" : "Invitado";
    JOptionPane.showMessageDialog(rootPane, 
        "Usuario de " + tipoUsuario + " creado exitosamente");

    // Debug - mostrar usuarios registrados
    System.out.println("\nNuevo usuario registrado:");
    System.out.println("Usuario: " + usuario + " | Perfil: " + perfil);
